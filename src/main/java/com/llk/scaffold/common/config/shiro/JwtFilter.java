...
import com.llk.scaffold.common.utils.JsonConvertUtil;
...
/**
 * @className: JwtFilter
 * @description: 过滤token, 并进行身份认证 \r
 *               代码的执行流程 preHandle->isAccessAllowed->executeLogin
 * @author: LiuLukuan
 * @date: 2019/11/11 0:04
 */
@Slf4j
public class JwtFilter extends BasicHttpAuthenticationFilter {
    ...
    private boolean refreshToken(ServletRequest request, ServletResponse response) {
        ...
        String tokenKey = RedisUtil.KEY_PREFIX + RedisUtil.USER_KEY_PREFIX + account;
        if (RedisUtil.hasKey(tokenKey)) {
            // Redis中RefreshToken还存在，获取RefreshToken的时间戳
            String currentTimeMillisRedis = RedisUtil.get(tokenKey).toString();
            // 获取当前AccessToken中的时间戳，与RefreshToken的时间戳对比，如果当前时间戳一致，进行AccessToken刷新
            if (JwtUtil.getClaim(token, JwtUtil.CURRENT_TIME_MILLIS).equals(currentTimeMillisRedis)) {
                // 获取当前最新时间戳
                String currentTimeMillis = String.valueOf(System.currentTimeMillis());
                // 设置RefreshToken中的时间戳为当前最新时间戳，且刷新过期时间重新为30分钟过期(JwtUtil可配置refreshTokenExpireTime属性)
                RedisUtil.set(tokenKey, currentTimeMillis, JwtUtil.REFRESH_TOKEN_EXPIRE_TIME);
